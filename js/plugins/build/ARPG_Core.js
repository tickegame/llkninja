...
(() => {
    ...
    PluginManager.registerCommand(ARPG_CorePluginName, "SetupFieldObject", function() {
        const event = this.event();
        if (!event)
            return;
    ...
    var _Sprite_CursorArrow = class extends Sprite {
        ...
        updateAnimation() {
            ...
            if (this._animationTime >= _Sprite_CursorArrow.MAX_ANIMATION_TIME) {
                this._animationSign = -1;
            } else if (this._animationTime <= -_Sprite_CursorArrow.MAX_ANIMATION_TIME) {
                this._animationSign = 1;
            }
        }
        createBitmap() {
    ...
    var _Sprite_TargetSelect = class extends Sprite {
        initialize() {
            ...
            this._mover = new SpriteMover(this, 1);
        }
        update() {
            super.update();
            this._mover.update();
    ...
    var ChangeActorProcess = class extends Component {
        ...
        forceChangeNextActor() {
            const changed = this.changeNextActorInternal();
            if ($gamePlayer.isBattler()) {
        ...
        changeNextActorInternal() {
            for (let i = 0; i < $gameParty.members().length - 1; i++) {
                for (let j = i + 1; j < $gameParty.members().length; j++) {
                    if ($gameParty.members()[j].isDead())
                        continue;
    ...
    var ARPG_Effect = class {
        ...
        applyToTarget() {
            ...
            if (ARPG_CorePluginParams.UseDamagePopup == null || ARPG_CorePluginParams.UseDamagePopup) {
                $gameTemp.requestFieldDamagePopup(this._target.character(), result.actionResult());
            }
    ...
    var CommonEventComponent = class extends Component {
        ...
        start() {
            ...
            if (this._character instanceof Game_Event) {
                this._interpreter.setup(commonEventData.list, this._character.eventId());
            } else {
    ...
    var BattlerCommonEventComponent = class extends CommonEventComponent {
        ...
        start() {
            ...
            if (this._character instanceof Game_Event) {
                ...
                this._interpreter.setCommonVariableValue(ARPG_CorePluginParams.BattlerSetting.UserEventIdCommonVariableId, this._character.eventId());
    ...
    var ARPG_Battler = class extends Component {
        ...
        checkSuccessGuard(damageEffect) {
            ...
            if (this.isGuarding()) {
                ...
                if (damageDir4 == null) {
                    ...
                    if (this._character.direction() === this._character.reverseDir(damageDir4)) {
                        successGuard = true;
                    }
                }
            }
        ...
        useSkill(skillOrItem, idOrName, opt = {}) {
            ...
            if (!this.canUsableSkill()) {
                if (queing) {
                    this._requestUseSkillQueue.push({ skillOrItem, idOrName, isConsumeCost });
                }
                return;
            }
        ...
        stopCharacterMove(reason) {
            if (this._character instanceof Game_Event) {
                this._character.stopSelfMovement(reason);
            } else if (this._character instanceof Game_Player) {
                this._character.stopMoveByInput(reason);
            }
        }
        resumeCharacterMove(reason) {
            if (this._character instanceof Game_Event) {
                this._character.resumeSelfMovement(reason);
            } else if (this._character instanceof Game_Player) {
                this._character.resumeMoveByInput(reason);
            }
        }
        ...
        applyCollideDamageEffectToTarget(target, arpgSkill) {
            if (target.isNoDamage())
                return;
            const effect = new ARPG_Effect(this, target, arpgSkill, Degree.fromDirection(this.character().direction()));
    ...
    var BattlerDeadComponent = class extends Component {
        ...
        start() {
            ...
            if (this._character instanceof Game_Player) {
                this._character.stopMoveByInput("dead");
            } else if (this._character instanceof Game_Event) {
                this._character.interpreter()?.lock("dead");
                this._character.stopSelfMovement("dead");
            }
            this._character.cancelAcceleration();
        ...
        terminate() {
            super.terminate();
            if (this._character instanceof Game_Player) {
                this._character.resumeMoveByInput("dead");
            } else if (this._character instanceof Game_Event) {
                this._character.interpreter()?.unlock("dead");
                this._character.resumeSelfMovement("dead");
            }
        }
    };
    ...
    var CharacterBlowAwayProcessor = class extends Processor {
        ...
        start() {
            super.start();
            if (this._character instanceof Game_Event) {
                this._character.interpreter()?.lock("knockback");
    ...
    var ARPG_SkillObject = class extends Component {
        ...
        applyDamageEffectToBattler(battler, arpgSkill) {
            ...
            this._character.setExSelfSwitchValue(ARPG_CorePluginParams.SkillObjectSetting.CollisionDetectExSelfSwitchId, true);
    ...
    var SkillObjectPositionController = class extends Component {
        ...
        update() {
            super.update();
            const skillObjectEvent = this._skillObject.character();
            const currentUserPosition = this._skillObject.user().character().positionPoint();
