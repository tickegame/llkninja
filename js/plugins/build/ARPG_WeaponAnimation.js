/*!/*:
@target MZ
@plugindesc ARPG_WeaponAnimation v1.0.2
@author unagi ootoro
@base ARPG_Core
...
【Required Plug-ins】
The "ARPG_Core.js" plugin is required to use this plugin.
...
/*!/*:ja
@target MZ
@plugindesc ARPG武器振りアニメーション v1.0.2
@author うなぎおおとろ
@base ARPG_Core
...
【必須プラグイン】
本プラグインを使用するには「ARPG_Core.js」プラグインが必要です。
...
(() => {
    ...
    var SwordAttackProcessor = class extends import_Processor.Processor {
        ...
        *process() {
            const sprite = this._character.getSprite();
            sprite.createMapWeaponSprite(this._weaponImageId);
            const weaponSprite = sprite._weaponSprite;
            weaponSprite.show();
            switch (this._character.direction()) {
                case 8:
                    yield* this.upAttack(weaponSprite);
                    break;
                case 6:
                    yield* this.rightAttack(weaponSprite);
                    break;
                case 2:
                    yield* this.downAttack(weaponSprite);
                    break;
                case 4:
                    yield* this.leftAttack(weaponSprite);
                    break;
                default:
                    throw new Error(`${this._character.direction()} is not found.`);
            }
            sprite.deleteMapWeaponSprite();
            this._character.removeDisableMoveReason("normalAttack");
        }
        *upAttack(weaponSprite) {
            const req = { id: -1, offsetX: -20, offsetY: -20, angle: 90 };
            this.showAnimation(req);
            weaponSprite.inner.x = 4;
            weaponSprite.inner.y = -40;
            for (let angle = 360 + 90; angle >= 270; angle -= 16) {
                weaponSprite.rotation = new import_Degree.Degree(angle - 45).toRad();
                yield;
            }
        }
        *rightAttack(weaponSprite) {
            const req = { id: -1, mirror: true };
            this.showAnimation(req);
            weaponSprite.inner.x = 0;
            weaponSprite.inner.y = -10;
            for (let angle = 0; angle <= 180; angle += 16) {
                weaponSprite.rotation = new import_Degree.Degree(angle - 45).toRad();
                yield;
            }
        }
        *downAttack(weaponSprite) {
        ...
        *leftAttack(weaponSprite) {
        ...
        showAnimation(request) {
            ...
            const animation = {
                ...
                flashTimings: [],
                name: "\u65AC\u6483<particle: Sprite_SwordAnimation>",
                offsetX: 0,
                offsetY: 0,
                rotation: {
                    x: 0,
            ...
            SceneManager._scene._spriteset.createAnimationSprite([this._character], animation, mirror, 0);
        }
    };
    ...
    Game_Character.prototype.showWeaponMotion = function(weaponImageId, se) {
        ...
        if (!this.hasComponent(this._weaponMotionComponent)) {
            this._weaponMotionComponent = new SwordAttackProcessor(weaponImageId);
